...
export default function Dashboard() {
  ...
  const [isRedeemingAmazon, setIsRedeemingAmazon] = useState(false);
  ...
  const handleRedeemAmazon = async () => {
    setIsRedeemingAmazon(true);
    setLogs((prev) => [
      {
        time: new Date().toLocaleTimeString(),
        message: "🛒 Starting Amazon gift card redemption...",
      },
      ...prev,
    ]);
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        const errorMsg = data.error || "Amazon redemption failed";
      ...
      if (data.bill) {
        ...
        setLogs((prev) => [
          {
            time: new Date().toLocaleTimeString(),
            message: `✓ Amazon gift card redeemed! Amount: $${data.bill.amount.toFixed(2)}`,
          },
          ...prev,
        ]);
      }
      ...
      setLogs((prev) => [
        {
          time: new Date().toLocaleTimeString(),
          message: `🎉 Amazon gift card redemption complete!`,
        },
        ...prev,
      ]);
    } catch (error: any) {
      console.error("Error redeeming Amazon gift card:", error);
      setLogs((prev) => [
        {
          time: new Date().toLocaleTimeString(),
          message: `⚠ Amazon redemption failed: ${error?.message || "Unknown error"}`,
        },
        ...prev,
      ]);
    } finally {
      setIsRedeemingAmazon(false);
    }
  };
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <header className="border-b border-slate-800 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-medium text-slate-100 hover:bg-orange-400 transition disabled:opacity-60 disabled:cursor-not-allowed"
            onClick={handleRedeemAmazon}
            disabled={isRedeemingAmazon}
          >
            {isRedeemingAmazon ? "Redeeming..." : "Redeem Amazon"}
          </button>
          <button
